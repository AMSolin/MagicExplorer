        ...
        if 's_selected_lists' not in st.session_state:
            ...
            def list_callback():
                ...
                if col == 'open':
                    ...
                    update_table(
                        'import_list', col, int(val), list_id, db_path='temp/temp_db.db'
                    )
            ...
            _, col1, col2, col3 = st.columns([0.04, 0.3, 0.25, 0.41])
            _ = col1.checkbox(
                'Select all',
            ...
            discard_button = col3.button('Discard import', type='primary')
            if discard_button:
                del st.session_state.s_selected_lists, st.session_state.current_list_id
                st.rerun()
            import_button = col2.button('Import')
            if import_button:
